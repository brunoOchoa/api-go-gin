...
import (
	...
	"github.com/brunoOchoa/api-rest-gin/controllers"
	"github.com/brunoOchoa/api-rest-gin/database"
	"github.com/brunoOchoa/api-rest-gin/models"
...
func TestVerificaStatusCodeDaSaudacaoComParametro(t *testing.T) {
	...
	mockDaRespsta := `{"API diz:":"E ai bruno, tudo beleza ?"}`
	respostaBody, _ := ioutil.ReadAll(resposta.Body)
	assert.Equal(t, mockDaRespsta, string(respostaBody))
}

func CriaAlunoMock() {
	aluno := models.Aluno{Nome: "Nome do Aluno Teste", CPF: "00000000000", RG: "000000000"}
...
func TestBuscaAlunoPorIDHandler(t *testing.T) {
	...
	assert.Equal(t, "000000000000", alunoMock.CPF)
	assert.Equal(t, "000000000", alunoMock.RG)
	assert.Equal(t, http.StatusOK, resposta.Code)
}

func TestDeletaAluboHandler(t *testing.T) {
	database.ConectaComBancoDeDados()
	CriaAlunoMock()
	r := SetupDasRotasDeTeste()
...
func TestEditaUmAlunoHandler(t *testing.T) {
	...
	assert.Equal(t, "99999999999", alunoMockAtualizado.CPF)
	assert.Equal(t, "999999999", alunoMockAtualizado.RG)
